...
class SplitStep(Method):
    ...
    def run(self, initial_wavefunction, total_time, dt, store_steps=1):
        """
        Ψ(r, t+dt) = U_r(dt/2) F^-1[ U_k(dt) F[ U_r(dt/2) Ψ(r, t) ] ]
        """
        ...
        if isinstance(self.simulation.H.particle_system, SingleParticle):
            Ψ = np.zeros(
                (store_steps + 1, *([self.H.N] * self.H.ndim)), dtype=np.complex128
            )
        ...
        # Ur = exp( 1/2 i * dt / hbar * V)
        Ur = np.exp(-0.5j * (self.simulation.dt / hbar) * np.array(self.H.Vgrid))

        # Uk = exp( 1/2 i * dt / hbar * p^2 / m)
        Uk = np.exp(-1j * (self.simulation.dt / hbar) * self.p2 / m)
        ...
        for i in bar(range(store_steps)):
            ...
            for j in range(Nt_per_store_step):
                ...
                # the above two lines are basically Ψ(t + dt) = Ur * F^-1[Uk * F[Ur * Ψ(t)]]
                # compare it to: Ψ(r, t+dt) = U_r(dt/2) F^-1[ U_k(dt) F[ U_r(dt/2) Ψ(r, t) ] ]
            Ψ[i + 1] = tmp
        print(f"Took {time.time() - t0:.2g} seconds. ")
...
class SplitStepCupy(Method):
    ...
    def run(self, initial_wavefunction, total_time, dt, store_steps=1):
        ...
        Ψ = cp.zeros(
            (store_steps + 1, *([self.H.N] * self.H.ndim)), dtype=cp.complex128
        )
