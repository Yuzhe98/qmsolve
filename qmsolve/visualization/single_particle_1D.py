...
class VisualizationSingleParticle1D(Visualization):
    ...
    def plot_eigenstate(self, k, xlim=None, show_imaginary_part=False):
        ...
        grid = plt.GridSpec(
            2, 2, width_ratios=[4.5, 1], height_ratios=[1, 1], hspace=0.1, wspace=0.2
        )
        ...
        if show_imaginary_part == True:
            eigenstate_plot1 = ax1.plot(
                x / Å, np.real(eigenstates_array[k]), label="$Re|\\psi(x)|$"
            )
            eigenstate_plot2 = ax1.plot(
                x / Å, np.imag(eigenstates_array[k]), label="$Im|\\psi(x)|$"
            )
            eigenstate_plot3 = ax1.plot(
                x / Å, np.abs(eigenstates_array[k]), label="$|\\psi(x)|$", color="white"
            )
            ax1.legend()
        else:
    ...
    def slider_plot(self, xlim=None, show_imaginary_part=False):
        ...
        grid = plt.GridSpec(
            2, 2, width_ratios=[5, 1], height_ratios=[1, 1], hspace=0.1, wspace=0.2
        )
        ax1 = fig.add_subplot(grid[0:2, 0:1])
        ax2 = fig.add_subplot(grid[0:2, 1:2])
        ...
        if show_imaginary_part == True:
            (eigenstate_plot1,) = ax1.plot(
                x / Å, np.real(eigenstates_array[0]), label="$Re|\\psi(x)|$"
            )
            (eigenstate_plot2,) = ax1.plot(
                x / Å, np.imag(eigenstates_array[0]), label="$Im|\\psi(x)|$"
            )
            (eigenstate_plot3,) = ax1.plot(
                x / Å, np.abs(eigenstates_array[0]), label="$|\\psi(x)|$", color="white"
            )
            ax1.legend()
        else:
    ...
    def slider_plot_earth(self, xlim=None, show_imaginary_part=False):
        plt.style.use("dark_background")
        ...
        if show_imaginary_part == True:
            (eigenstate_plot1,) = ax1.plot(
                x / earth_radius, np.real(eigenstates_array[0]), label="$Re|\\psi(x)|$"
            )
            (eigenstate_plot2,) = ax1.plot(
                x / earth_radius, np.imag(eigenstates_array[0]), label="$Im|\\psi(x)|$"
            )
            (eigenstate_plot3,) = ax1.plot(
                x / earth_radius, np.abs(eigenstates_array[0]), label="$|\\psi(x)|$", color="white"
            )
            ax1.legend()
        else:
            (eigenstate_plot,) = ax1.plot(x / earth_radius, np.real(eigenstates_array[0]))
    ...
    def animate(
        ...
        def func_animation(*arg):
            ...
            if (animation_data["n"] % 1.0) > 0.5:
                ...
                wavefunction = (
                    np.cos(np.pi * transition_time) * eigenstates_array[state]
    ...
    def superpositions(self, states, fps=30, total_time=20, **kw):
        ...
        animation_data = {
            "ticks": 0,
            "norm": get_norm_factor(eigenstates[0]),
            "is_paused": False,
        }
        ...
        params = {
            "dt": 0.001,
            "xlim": [-self.eigenstates.extent / 2.0, self.eigenstates.extent / 2.0],
            "save_animation": False,
            "frames": 120,
        }
        for k in kw.keys():
        ...
        (line3,) = ax.plot(
            x / Å, np.abs(eigenstates[0]), label="$|\\psi(x)|$", color="white"
        )
