...
class VisualizationIdenticalParticles1D(Visualization):
    ...
    def plot_eigenstate(self, k, xlim=None):
        ...
        grid = plt.GridSpec(
            2, 2, width_ratios=[4.5, 1], height_ratios=[2.5, 1], hspace=0.4, wspace=0.4
        )
        ax1 = fig.add_subplot(grid[0:1, 0:1])
        ax3 = fig.add_subplot(
            grid[1:2, 0:1], sharex=ax1
        )  # probability density of finding any particle at x
        ...
        im = ax1.imshow(
            complex_to_rgb(eigenstates_array[k] * np.exp(1j * 2 * np.pi / 10 * k)),
            aspect="auto",
            origin="lower",
            extent=[-L, L, -L, L],
        ...
        prob_density = np.abs(
            np.sum((eigenstates_array[k]) * np.conjugate(eigenstates_array[k]), axis=1)
        )
    ...
    def slider_plot(self, xlim=None):
        ...
        L = self.eigenstates.extent / 2 / Å
        eigenstate_plot = ax1.imshow(
            complex_to_rgb(eigenstates_array[0] * np.exp(1j * 2 * np.pi / 10 * 0)),
            aspect="auto",
            origin="lower",
            extent=[-L, L, -L, L],
            interpolation="bilinear",
        ...
        prob_density = np.abs(
            np.sum((eigenstates_array[0]) * np.conjugate(eigenstates_array[0]), axis=1)
        )
        prob_plot = ax3.plot(x, prob_density, color="cyan")
        prob_plot_fill = ax3.fill_between(x, prob_density, alpha=0.1, color="cyan")
        ...
        def update(state):
            state = int(state)
            eigenstate_plot.set_data(
                complex_to_rgb(
                    eigenstates_array[state] * np.exp(1j * 2 * np.pi / 10 * state)
                )
            )
            ...
            prob_density = np.abs(
                np.sum(
                    (eigenstates_array[state]) * np.conjugate(eigenstates_array[state]),
                    axis=1,
                )
            )
            prob_plot = ax3.plot(x, prob_density, color="cyan")
    ...
    def slider_plot_earth(self, xlim=None):
        ...
        ax1.set_xlabel("$x_1$ [earth_radius]")
        ax1.set_ylabel("$x_2$ [earth_radius]")
        ax1.set_title("$\\Psi(x_1,x_2)$")
        ...
        ax3.set_xlabel("$x$ [earth_radius]")
        ax3.set_ylabel("${\\| \\Psi(x)\\|}^{2} $")
        ax3.set_title("Probability density")
        plt.setp(ax3.get_yticklabels(), visible=False)
        if xlim != None:
            ax1.set_xlim(np.array(xlim) / earth_radius)
            ax3.set_xlim(np.array(xlim) / earth_radius)
            ax1.set_ylim(np.array(xlim) / earth_radius)
            ax3.set_ylim(np.array(xlim) / earth_radius)

        E0 = energies[0]
        for E in energies:
            ax2.plot([0, 1], [E, E], color="gray", alpha=0.5)

        L = self.eigenstates.extent / 2 / earth_radius
        eigenstate_plot = ax1.imshow(
            complex_to_rgb(eigenstates_array[0] * np.exp(1j * 2 * np.pi / 10 * 0)),
            aspect="auto",
            origin="lower",
    ...
    def animate(
        ...
        L = self.eigenstates.extent / 2 / Å
        eigenstate_plot = ax1.imshow(
            complex_to_rgb(eigenstates_array[0]),
            aspect="auto",
            origin="lower",
            extent=[-L, L, -L, L],
            interpolation="bilinear",
        ...
        prob_density = np.sum(
            (eigenstates_array[0]) * np.conjugate(eigenstates_array[0]), axis=1
        )
        ...
        def func_animation(frame):
            ...
            if (animation_data["n"] % 1.0) > 0.5:
                ...
                eigenstate_combination = np.cos(
                    np.pi * transition_time
                ) * eigenstates_array[state] * np.exp(
                    1j * 2 * np.pi / 10 * state
                ) + np.sin(
                    np.pi * transition_time
                ) * eigenstates_array[
                    (state + 1) % len(energies)
                ] * np.exp(
                    1j * 2 * np.pi / 10 * (state + 1)
                )
                ...
                prob_density = np.abs(
                    np.sum(
                        eigenstate_combination * np.conjugate(eigenstate_combination),
                        axis=1,
                    )
                )
                ...
                (prob_plot,) = ax3.plot(x, prob_density, color="cyan")
                prob_plot_fill = ax3.fill_between(
                    x, prob_density, alpha=0.1, color="cyan"
                )
                ...
                E = (
                    E_N * np.cos(np.pi * transition_time) ** 2
                    + E_M * np.sin(np.pi * transition_time) ** 2
                )
                line.set_ydata([E, E])
            else:
                line.set_ydata([energies[state], energies[state]])

                eigenstate_combination = eigenstates_array[int(state)] * np.exp(
                    1j * 2 * np.pi / 10 * state
                )
                eigenstate_plot.set_data(complex_to_rgb(eigenstate_combination))
        ...
        a = animation.FuncAnimation(
            fig, func_animation, blit=True, frames=total_frames, interval=1 / fps * 1000
        )
    ...
    def superpositions(self, states, fps=30, total_time=20, **kw):
        params = {
            "dt": 0.001,
            "xlim": [-self.eigenstates.extent / 2 / Å, self.eigenstates.extent / 2 / Å],
            "ylim": [-self.eigenstates.extent / 2 / Å, self.eigenstates.extent / 2 / Å],
            "save_animation": False,
            "hide_controls": False,
        ...
        if isinstance(states, int) or isinstance(states, float):
            coeffs = np.array(
                [1.0 if i == 0 else 0.0 for i in range(states)], dtype=np.complex128
            )
        ...
        X, Y = np.meshgrid(
            np.linspace(-1.0, 1.0, eigenstates[0].shape[0]),
        ...
        def make_update(n):
            def update(phi, r):
                ...
                psi = np.dot(coeffs, eigenstates.reshape([states, N * N]))
                psi = psi.reshape([N, N])
            ...
            for i in range(states):
                if states <= 30:
                    ...
                    circle_ax = fig.add_subplot(
                        grid[8 if i < 30 else 9, i if i < 30 else i - 30],
                        projection="polar",
                    )
                    circle_ax.set_title(
                        "n=" + str(i),  # + '\nE=' + str() + '$E_0$'
        ...
        def func(*args):
            ...
            psi = np.dot(coeffs * norm_factor, eigenstates.reshape([states, N * N]))
            psi = psi.reshape([N, N])
        ...
        a = animation.FuncAnimation(
            ...
            blit=True,
            interval=1 / fps * 1000,
            frames=None if (not params["save_animation"]) else total_frames,
        )
...
class TimeVisualizationTwoIdenticalParticles1D(TimeVisualization):
    ...
    def plot(
        ...
        time_ax = ax2.text(
            0.97, 0.97, "", color="white", transform=ax2.transAxes, ha="right", va="top"
        )
        time_ax.set_text("t = {} femtoseconds".format("%.2f" % (t / femtoseconds)))
        ...
        ax1.imshow(
            complex_to_rgba(
                self.simulation.Ψ_plot[index], max_val=wavefunction_saturation
            ),
            origin="lower",
            interpolation="bilinear",
            extent=[-L / 2, L / 2, -L / 2, L / 2],
        ...
        prob_density = np.abs(
            np.sum(
                (self.simulation.Ψ_plot[index])
                * np.conjugate(self.simulation.Ψ_plot[index]),
                axis=1,
            )
        )
    ...
    def animate(
        ...
        wavefunction_plot = ax1.imshow(
            complex_to_rgba(self.simulation.Ψ_plot[0], max_val=wavefunction_saturation),
            origin="lower",
            interpolation="bilinear",
            extent=[-L / 2, L / 2, -L / 2, L / 2],
        )
        ...
        time_ax.set_text("t = {} femtoseconds".format("%.3f" % (0.00 / femtoseconds)))
        ...
        animation_data = {
            ...
            "ax2": ax2,
            "frame": 0,
            "max_prob_density": max(prob_density) * 1.3,
        }
        ...
        def func_animation(*arg):
            ...
            index = int(
                (self.simulation.store_steps)
                / self.simulation.total_time
                * animation_data["t"]
            )

            wavefunction_plot.set_data(
                complex_to_rgba(
                    self.simulation.Ψ_plot[index], max_val=wavefunction_saturation
                )
            )
